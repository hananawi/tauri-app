...
impl Ocr {
    pub fn capture_screen(&self, rect: CGRect) {
        ...
        unsafe {
            ...
            let shot_cb = RcBlock::new(move |img_ptr: *mut CGImage, err_ptr: *mut NSError| {
                if !err_ptr.is_null() {
                    ...
                    let _ = completion_tx.send(Err("截图失败：系统返回错误".to_string()));
                    return;
                }

                if img_ptr.is_null() {
                    eprintln!("Screenshot returned null image pointer");
                    let _ = completion_tx.send(Err("截图失败：未获取到图像".to_string()));
                    return;
                }

                let img_ref = &*img_ptr;
                let bitmap = NSBitmapImageRep::initWithCGImage(NSBitmapImageRep::alloc(), img_ref);
                ...
                let Some(png_data) =
                    ...
                    let _ = completion_tx.send(Err("截图失败：无法编码图像".to_string()));
                    return;
                };
                ...
                if !pasteboard.setData_forType(Some(&png_data), NSPasteboardTypePNG) {
                    ...
                    let _ = completion_tx.send(Err("截图失败：无法写入剪切板".to_string()));
                    return;
                }
        ...
        match rx.recv() {
            ...
            Err(recv_err) => eprintln!("Failed to receive screenshot result: {recv_err}"),
        }
    }
}
