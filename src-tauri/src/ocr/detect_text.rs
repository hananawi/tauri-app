...
impl Ocr {
    pub fn detect_text(&self, rect: CGRect) -> Vec<String> {
        ...
        let detected_texts: Arc<Mutex<Vec<String>>> =
            ...
            let shot_cb = RcBlock::new(
                move |img_ptr: *mut CGImage, err_ptr: *mut NSError| {
                    ...
                    let ocr_cb = RcBlock::new(
                        move |req_ptr: NonNull<VNRequest>,
                              err_ptr: *mut NSError| {
                            if !err_ptr.is_null() {
                    ...
                    ocr_req.setRecognitionLanguages(
                        &objc2_options.target_languages,
                    );

                    if let Err(error) = handler.performRequests_error(
                        &NSArray::from_retained_slice(&[ocr_req
                            .into_super()
                            .into_super()]),
                    ) {
